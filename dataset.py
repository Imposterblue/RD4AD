...
class MVTecDataset(torch.utils.data.Dataset):
    def __init__(self, root, transform, gt_transform, phase, defect_type=None, num = '000'): #defect_type, num 추가
        if phase == 'train':
        ...
        # load dataset
        self.img_paths, self.gt_paths, self.labels, self.types = self.load_dataset(defect_type, num)  # self.labels => good : 0, anomaly : 1

    def load_dataset(self,defect_type, num):
        ...
        if defect_type == 'good':
            img_paths = glob.glob(os.path.join(self.img_path, defect_type) + f"/{num}.png")
            ...
            img_paths = glob.glob(os.path.join(self.img_path, defect_type) + f"/{num}.png")
            gt_paths = glob.glob(os.path.join(self.gt_path, defect_type) + f"/{num}_mask.png")
            #img_paths.sort()
            #gt_paths.sort()
            img_tot_paths.extend(img_paths)
